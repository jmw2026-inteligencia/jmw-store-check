...
# --- CSS INTEGRAL BLINDADO ---
st.markdown("""
    <style>
    .stApp { background-color: #367c87 !important; }
    h1, .stMarkdown, label, .stExpander { color: #ffffff !important; }
    /* Selector y Input: fondo naranja claro y texto oscuro */
    div[data-baseweb="select"] > div, div[data-baseweb="base-input"], .stNumberInput input { 
        background-color: #fff2e6 !important; 
        color: #000000 !important; 
    }
    /* Elimina el fondo negro de la lista desplegable */
    div[role="listbox"] { background-color: #fff2e6 !important; }
...
# 3. Producto (Limpieza controlada con keys en session_state)
with st.expander("📦 3. Producto y Precio", expanded=True):
    busqueda = st.text_input("Buscar Producto:", key="busqueda_key")
    df_filtrado = df_maestro[df_maestro['nombre'].str.contains(busqueda, case=False, na=False)] if busqueda else df_maestro
    prod_sel = st.selectbox("Seleccionar", df_filtrado['nombre'].tolist(), index=None, key="prod_sel_key")
    es_usd = st.toggle("¿Precio en DÓLARES?", True)
    precio = st.number_input("Precio Marcado", min_value=0.0, step=0.01, key="precio_input_key")
...
if st.button("🚀 TRANSMITIR REGISTRO"):
    if not prod_sel:
        ...
        if res.status_code in [200, 201, 204]:
            ...
            # LIMPIEZA TOTAL SIN ERRORES
            for key in ['busqueda_key', 'prod_sel_key', 'precio_input_key', 'serial_input']:
                st.session_state[key] = None if key == 'prod_sel_key' else ""
            time.sleep(1)
            st.rerun()
        else:
