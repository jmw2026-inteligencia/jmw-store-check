...
# --- 2. CARGA FORZADA DEL ARCHIVO ---
def cargar_maestro():
    # Buscamos en el directorio actual
    ruta_archivo = os.path.join(os.getcwd(), "IMPORTACION_ANALISIS_PRECIO.xlsx")
    return pd.read_excel(ruta_archivo, sheet_name="PRODUCTOS")

df_maestro = cargar_maestro()
df_maestro.columns = [str(c).lower().strip() for c in df_maestro.columns]

# --- 3. FORMULARIO CON LIMPIEZA ---
...
if st.button("🚀 TRANSMITIR REGISTRO"):
    if producto_sel == "-- Seleccione --":
        ...
        if res.status_code in [200, 201, 204]:
            st.success("✅ ¡Registro enviado! Limpiando...")
